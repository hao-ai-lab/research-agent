...
class WildStepPlan(BaseModel):
    """A single step in the wild loop plan."""
    step_number: int
    goal: str          # What this step should accomplish
    status: str = "pending"  # pending | in_progress | done
    summary: str = ""  # Filled when step completes


class WildPlan(BaseModel):
    """Plan generated by the planning stage."""
    steps: list = []           # List of WildStepPlan dicts
    current_step: int = 0      # Index into steps (0-based)


class BuildPromptRequest(BaseModel):
    """Request to build a wild loop prompt server-side."""
    prompt_type: str                              # "planning" | "exploring" | "run_event" | "alert" | "analysis"
    # Common
    goal: Optional[str] = None
    iteration: Optional[int] = None
    max_iterations: Optional[int] = None
    step_goal: Optional[str] = None               # Per-step goal from the plan
    # run_event fields
...
wild_loop_state: dict = {
    "phase": "idle",
    "stage": "exploring",     # planning | exploring | running | analyzing
    "is_active": False,
    "iteration": 0,
    "goal": None,
    "session_id": None,
    ...
    "is_paused": False,
    "sweep_id": None,
    "plan": None,             # WildPlan dict or None
    "step_goal": None,        # Current step goal string (for display)
    "termination": {
        "max_iterations": None,
...
_FALLBACK_PLANNING = """# Wild Loop — Planning Stage
...
You are starting a new autonomous experiment loop. Before taking action, produce a step-by-step plan.

Your plan should have 3-8 concrete steps. Each step should be:
- **Specific**: Clear what needs to happen
- **Verifiable**: Has a way to check it's done
...
## What You Should Do
1. Focus on the Current Step above
2. Explore the codebase and understand what experiments are needed
...
def build_prompt_for_frontend(
    ...
    step_goal = req.step_goal or wild_loop_state.get("step_goal") or goal
    ...
    if prompt_type == "planning":
        variables["experiment_context"] = req.experiment_context or "No experiment state yet."
...
def build_wild_prompt(prompt_skill_render_fn: Callable, experiment_context: str) -> str:
    ...
    step_goal = wild_loop_state.get("step_goal") or goal
...
def parse_plan(text: str) -> Optional[list]:
    """Parse a <plan>[...]</plan> JSON tag from the agent's response.

    Returns a list of dicts with keys: step, goal, or None.
    """
    m = re.search(r"<plan>([\s\S]*?)</plan>", text)
    if not m:
        return None
    try:
        steps = json.loads(m.group(1).strip())
        ...
        for s in steps:
            if isinstance(s, dict) and s.get("goal"):
                validated.append({
                    "step": s.get("step", len(validated) + 1),
                    "goal": s["goal"],
                })
        return validated if validated else None
    except (json.JSONDecodeError, TypeError) as err:
        logger.warning("Failed to parse plan: %s", err)
        return None
...
def parse_next_step(text: str) -> Optional[str]:
    ...
    Returns the next step description string, or None.
    """
    m = re.search(r"<next_step>([\s\S]*?)</next_step>", text)
    if not m:
        return None
    step_text = m.group(1).strip()
    return step_text if step_text else None
...
class WildLoopEngine:
    ...
    def start(self, req: WildStartRequest) -> dict:
        ...
        # Enqueue the planning prompt (iteration 0 = planning)
        self._enqueue_planning_prompt()
        ...
        logger.info("[wild-engine] Started with planning: goal=%s session=%s", req.goal, req.session_id)
        return wild_loop_state
    ...
    def on_response_complete(self, response_text: str) -> dict:
        ...
        # Step advancement: check for <next_step> tag (for exploring/analyzing)
        if stage in ("exploring", "analyzing"):
            self._try_advance_step(response_text)
    ...
    def _handle_planning_response(self, response_text: str, iteration: int):
        """Handle response from the planning stage.

        Parses a <plan>[...]</plan> tag, stores the plan, and transitions
        to exploring with step 1's goal.
        """
        ...
        if plan_steps:
            ...
            wild_loop_state["plan"] = plan

            # Set step goal to the first step
            first_goal = plan["steps"][0]["goal"] if plan["steps"] else wild_loop_state.get("goal", "")
            wild_loop_state["step_goal"] = first_goal
            plan["steps"][0]["status"] = "in_progress"

            logger.info("[wild-engine] Plan created with %d steps, transitioning to exploring", len(plan["steps"]))
        else:
            # No valid plan found — use goal as single step
            logger.warning("[wild-engine] No valid plan parsed, using goal as step goal")
            wild_loop_state["step_goal"] = wild_loop_state.get("goal", "Continue working")

        # Transition to exploring stage
    ...
    def _get_current_step_goal(self) -> str:
        """Get the current step goal from the plan, or fall back to the main goal."""
        plan = wild_loop_state.get("plan")
        if plan and plan.get("steps"):
            current_idx = plan.get("current_step", 0)
            steps = plan["steps"]
            if 0 <= current_idx < len(steps):
                return steps[current_idx]["goal"]
        return wild_loop_state.get("step_goal") or wild_loop_state.get("goal", "Continue working")

    def _try_advance_step(self, response_text: str):
        """Check for <next_step> tag and advance the plan's current step."""
        next_step_desc = parse_next_step(response_text)
        if next_step_desc is None:
            return

        plan = wild_loop_state.get("plan")
        if not plan or not plan.get("steps"):
            # No plan — use the next_step as dynamic step goal
            wild_loop_state["step_goal"] = next_step_desc
            logger.info("[wild-engine] Dynamic step goal set: %s", next_step_desc[:80])
            return
        ...
        if 0 <= current_idx < len(steps):
            ...
            steps[current_idx]["summary"] = next_step_desc[:200]
        ...
        if next_idx < len(steps):
            ...
            wild_loop_state["step_goal"] = steps[next_idx]["goal"]
            logger.info("[wild-engine] Advanced to step %d: %s", next_idx + 1, steps[next_idx]["goal"][:80])
        else:
            # All plan steps complete — use agent's suggestion as dynamic goal
            wild_loop_state["step_goal"] = next_step_desc
            logger.info("[wild-engine] All plan steps done, dynamic goal: %s", next_step_desc[:80])

    def _enqueue_planning_prompt(self):
        """Build and enqueue the planning stage prompt."""
        goal = wild_loop_state.get("goal", "Continue working")

        # Build experiment context if callbacks available
        experiment_context = "No experiment state yet."
        if self._get_runs and self._get_sweeps and self._get_alerts:
            try:
                from server.wild_loop import build_experiment_context
                experiment_context = build_experiment_context(
                    self._get_runs(),
                    self._get_sweeps(),
                    self._get_alerts(),
                    lambda: None,  # recompute_fn not needed for planning
                )
            except Exception:
                pass

        # Try skill template first
        provenance = None
        prompt_text = None
        ...
        if not prompt_text:
            prompt_text = _render_simple(_FALLBACK_PLANNING, {
                "goal": goal,
                "experiment_context": experiment_context,
            })

        event = {
            "id": f"planning-{uuid.uuid4().hex[:8]}",
            "priority": 80,  # Higher priority than exploring (90), lower than alerts (20)
            "title": "Planning",
            "prompt": prompt_text,
            "display_message": "Planning step-by-step approach...",
            "type": "planning",
            "created_at": time.time(),
            "provenance": provenance,
        }
        ...
        self._pending_prompt = None
        logger.debug("[wild-engine] Enqueued planning prompt")

    def _enqueue_exploring_prompt(self, iteration: int):
        """Build and enqueue an exploring prompt."""
        goal = wild_loop_state.get("goal", "Continue working")
        step_goal = self._get_current_step_goal()
        ...
        event = {
            ...
            "prompt": prompt_text,
            "display_message": step_goal[:100] if step_goal != goal else None,
            "type": "exploring",
            "created_at": time.time(),
            "provenance": provenance,
        }
        ...
        self._pending_prompt = None
        logger.debug("[wild-engine] Enqueued exploring prompt iter=%d step_goal=%s", iteration, (step_goal or "")[:50])
