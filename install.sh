...
if [ "${1:-}" = "--dev" ]; then
  ...
  log ""
  log "============================================================"
  log "  ACTION REQUIRED: Activate Node 20 before continuing."
  log ""
  log "  Run:"
  log "    nvm use 20"
  log ""
  log "  This is needed because install.sh runs in a subshell,"
  log "  so the Node version change does not carry over."
  log "============================================================"
  log ""
  ...
  log ""
  log "  # Terminal 1 — OpenCode"
  log "  export RESEARCH_AGENT_USER_AUTH_TOKEN=\"\$(openssl rand -hex 16)\""
  log "  echo \$RESEARCH_AGENT_USER_AUTH_TOKEN  # store this for the GUI Settings page"
...
ensure_tmux_server() {
  run_tmux start-server >/dev/null 2>&1 || true
}
...
tmux_window_exists() {
  local session="$1"
  local window="$2"
  run_tmux list-windows -t "$session" -F '#W' 2>/dev/null | grep -Fxq "$window"
}
...
ensure_tmux_session() {
  ...
  if ! tmux_session_exists "$TMUX_SESSION_NAME"; then
    local tmux_err uid bootstrap_cmd
    # macOS /bin/sleep does not support "infinity"; keep this shell alive portably.
    bootstrap_cmd='while :; do sleep 3600; done'
    if ! tmux_err="$(run_tmux new-session -d -s "$TMUX_SESSION_NAME" -n bootstrap "$bootstrap_cmd" 2>&1)"; then
      uid="$(id -u 2>/dev/null || echo '?')"
      warn "tmux failed to create session ${TMUX_SESSION_NAME}: ${tmux_err}"
      die "Cannot start tmux session. Ensure /private/tmp/tmux-${uid} (macOS) or /tmp/tmux-${uid} is writable."
    fi
  fi
}
